    ...
    with patch("hepcrawl.settings.OXFORD_DOWNLOAD_DIR", download_dir):
        ...
        for test_file in test_files:
            ...
            for req in xml_requests:
                xml_path = fake_response_from_file(
                    req.meta["xml_url"].replace("file://", "")
                )
...
def test_copyrights(results):
    ...
    expected_results = (
        dict(
            copyright_statement="\xa9  The Author(s) 2019. Published by Oxford University Press on behalf of the "
...
def test_arxiv_eprints(results):
    ...
    expected_results = ([dict(value="1611.10151")],)
